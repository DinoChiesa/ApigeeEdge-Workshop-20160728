...
3. Use postman to obtain a new token via password grant, with those
...
4. Invoke this target: `GET /v1/oauth-resources/t2` passing in an authorization header . Example: 
...
5. Try with other usernames, and you will get tokens with different
  scopes. Run the same tests (/t1, /t2, /t3, /t4) with those other tokens to
  see how the behavior changes.

6. You can examine the API Proxy design inside the Apigee Edge Administrative UI.
